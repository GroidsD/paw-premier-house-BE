...
const cancelBooking = async ({
  ...
  try {
    ...
    if (booking.status !== "pending") throw new Error("Booking không thể huỷ");
...
const assignBookingToStaff = async ({ bookingId, staffId, scheduleId }) => {
  ...
  try {
    ...
    if (booking.status !== "pending") throw new Error("Booking không thể nhận");
    ...
    if (!scheduleStaff) throw new Error("Staff không thuộc ca hoặc đang bận");
